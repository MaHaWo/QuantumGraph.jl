...
# specs/migration-compatibility.feature
# Scenario: Distributed training APIs are not advertised as supported
@then("DDP, multi-node, or cluster training is documented as deferred or unsupported") do context
    @expect occursin(r"(?i)(DDP|multi[- ]node|cluster).*(deferred|unsupported|outside scope)|(?:deferred|unsupported|outside scope).*(DDP|multi[- ]node|cluster)", qg_compat_text(context))
end

# specs/migration-compatibility.feature
# Scenario: Distributed training APIs are not advertised as supported
@then("no public API is described as a supported distributed training entry point") do context
    @expect !occursin(r"(?i)supported\s+(DDP|distributed|multi[- ]node).*(API|entry point)", qg_compat_text(context))
end

# specs/migration-compatibility.feature
# Scenario: Distributed training requests fail with an intentional deferral error
@given("a workflow configuration requests DDP, multi-node, or distributed training") do context
    context[:distributed_request] = true
end
